...
export default function PaymentEntry() {
    ...
    const validate = () => {
        ...
        if (!headerData.RpDate) {
            errors.RpDate = 'Payment date is required';
        ...
        //     errors.Amount = 'Total allocated amount must match payment amount';
        //     showToast('Total allocated amount must match payment amount', "error");
        //     hasError = true;
        // }
    ...
    }, [selectedPaymentDate, selectedRefDate]);
    ...
    const getCode = async () => {
        const { lastNo, IsEditable } = await getLastNumber('PV');
        setCode(lastNo);
        setheaderData(prev => ({
            ...prev,
            RpNo: lastNo
    ...
    const loadInvoiceDetails = async (invoiceId) => {
        try {
            ...
            if (Success) {
                ...
                setSelectedPaymentDate(new Date(headData?.RpDate));
    ...
    return (
        <>
            <Helmet>
                <title>Sales Payment | Exapp</title>
            </Helmet>
                    ...
                    <Stack direction="row" justifyContent="flex-end" mb={2} mt={2}>
                        {isEditable && (
                            <Button variant="contained" color={isEditMode || id ? 'warning' : 'success'} size='large' onClick={handleSave}>
                                {isEditMode || id ? 'Update Payment' : 'Create Payment'}
                            </Button>
                        )}
                    </Stack>
                </Stack>
            ...
            <PrintDialog
                ...
                title="Payment Print Preview"
                printRef={printRef}
                documentTitle={`Payment Voucher-${headerData.RpNo}`}
            >
                <PaymentPrint
            ...
            >
                <DialogTitle>Confirm Paid From Change</DialogTitle>
                <DialogContent>
                    <Typography>
                        Changing the paid from account will remove all selected bills and journal details. Do you want to continue?
                    </Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => {
